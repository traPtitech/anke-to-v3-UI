...
export const formatFutureRelativeDate = (diff: number) => {
  ...
  if (diff < week) return `${Math.floor(diff / day)}日後`;
  if (diff < month) return `${Math.floor(diff / week)}週間後`;
  if (diff < year) return `${Math.floor(diff / month)}ヶ月後`;
  return `${Math.floor(diff / year)}年後`;
};
